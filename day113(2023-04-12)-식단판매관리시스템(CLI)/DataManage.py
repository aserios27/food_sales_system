...
from Filter import FindSql, SelOpt, sqlAction
...
# sales테이블 등록을 위한 연월일, 식단 코드 필터 
class FileTextFilter():
    def __init__(self):
        self.content = ''
        self.code = ''
        self.st = ''
# 2023-02-28 calender라이브러리 활용한 매서드로 변경 / *** 코드 참조        
# 연월일 필터 매서드
...
# 윤년, 월, 일 확인 
    ...
    def codeFilter(self, content):
        try:
            ...
            cursor = conn.cursor()    
            chk_ls = [content[i] for i in range(len(content)) if sqlAction(f"SELECT unitPrice FROM menu_planner WHERE pCode = '{content[i][0]}'")]
    ...
    def tableChk(self, contents):
        try:
            conn = pymysql.connect(**config)
            cursor = conn.cursor()
            sql = f"SELECT sCode, date_format(sDate,'%Y%m%d') as date, Qty FROM menu_sales ORDER BY seqNo"
        ...
        try:
            ...
            if content_duplicate_chk == 1:
                ...
                for i in range(len(content_code_chk)):
                    sql = f"SELECT unitPrice FROM menu_planner WHERE pCode = '{content_code_chk[i][0]}'"
                    rows = sqlAction(sql)
                    code = content_code_chk[i][0]   #--> 식단  코드
                    unitPrice = rows[0][0]          #--> 식단  가격
                    date = content_code_chk[i][1]   #--> 식단  날짜
                    qty = content_code_chk[i][2]    #--> 식단  수량
                    total_price = int(unitPrice) * int(qty)
                    if content_duplicate_chk == 0:
                        sql = f"INSERT INTO menu_sales(sCode, sDate, Qty, Amt) VALUES('{code}','{date}','{qty}','{total_price}')" 
                        sqlAction(sql, True)
...
#--------------------------      
# 데이터 시각화
# sales테이블의 전체 데이터를 가져온 후 년월일, 식단 코드를 필터링합니다.
...
class GraphsPreProcessing(FilePreProcessing, FileTextFilter):
    ...
    def saleDataCreate(self):
        while True:
            yesNo = input('menu_sales테이블을 텍스트파일로 저장하시겠습니까>(y/n) : ')
    ...
    def salePreProcessing(self):
        try:
            ...
            # 1) 년월일 필터 / 2023-03-03 년월일 필터 변경 적용 
            content_year_chk = [content for content in contents if FileTextFilter().dateFilter(content[1])]
            ...
            # 4) 데이터에 식단 명 추가후 데이터 프레임
            sql = "SELECT pCode, pName FROM menu_planner"
    ...
    def saleGraphShow(self, sel):
        ...
        # 2번 실행(모델 별 판매금액)
            ...
            plt.rc("font", family='Malgun Gothic')                    # 한글 폰트 설정
            plt.title("2023년 02월 식단 별 판매금액 합계")            # 그래프에 제목 넣기
            plt.bar(df_Name.index,df_Name['Amt'], label = '판매량')   # 막대그래프 생성
            plt.xticks(fontsize = 10, rotation=45)                    # x축 폰트 사이즈 및 눈금 회전
            plt.xlabel('식단 명')                                     # x축 눈금 회전
