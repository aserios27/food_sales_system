...
# (1) 판매기록 등록 매서드      
    ...
    #--------------------------   
    # (4) 세무자료용 코드별 판매금액 합계 매서드 / 2023-03-22
    def saleMinSum(self) : 
        try :
    ...
    #--------------------------   
    # (5) 주주총회용 코드별 판매금액 합계 매서드 / 2023-03-22        
    def saleMaxSum(self) : 
        try :
            print('조회할 ',end=''); in_code = UserInput().codeInput()
            sql = f"SELECT New_unitPrice FROM product_history WHERE pCode = '{in_code}'"
            rows = sqlAction(sql) ; max_unitPrice = int(max([i for row in rows for i in row]))
            sql = f"SELECT s.seqNo, s.sCode, p.pName, date_format(s.sDate,'%Y.%m.%d') as date, s.qty FROM sales AS s JOIN product AS p ON s.sCode = p.pCode WHERE p.pCode = '{in_code}'" 
            rows = sqlAction(sql) 
            print('===판매조회(코드별-주주총회용)===')
            print(f"{fmt('No',6,'l')}{fmt('제품코드',10,'l')}{fmt('제품명',20,'l')}{fmt('판매등록일자',22,'l')}{fmt('판매수량',16,'l')}{fmt('총 판매가격',5,'l')}") ; print('-'*90)
            sum_amt = 0
            for row in rows :
                max_amt = row[4] * max_unitPrice
                print(f"{fmt(row[0],6,'l')}{fmt(row[1],10,'l')}{fmt(row[2],20,'l')}{fmt(row[3],10,'l')}{float(fmt(row[4],7,'l')):20,.2f}{float(fmt(max_amt,5,'l')):20,.2f}")
                sum_amt += max_amt
            print('-'*90)
            print(f"{fmt('판매금액합계',66,'l')}{float(fmt(sum_amt,10,'l')):20,.2f}")
...
def startSale():
    while True:
        ...
        print("판매       수정             : 5  ")
        print("판매       삭제             : 6  ")
        print("판매조회(코드별-세무자료용) : 7  ")
        print("판매조회(코드별-주주총회용) : 8  ")
        print("판매  테이블  리셋          : 9  ")
        print("판매  관리 종료             : 99 ")
        print("시스템     종료             : 0 ")
