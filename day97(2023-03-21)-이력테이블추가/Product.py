...
from Filter import UserInput, FindSql, SelOpt, ShowTables, TableCreate, Read, TableReset,sqlAction
from History import HisFunc
            ...
            while True:
                ...
                if in_code != '' :
                    ...
                    #2) 같은 제품코드가 존재하지 않는 경우
                        ...
                        # history 테이블 데이터 추가
                        super().prodhisCreate(in_code, iValue)
                        print("제품등록을 성공했습니다.")
                        break
            ...
            while True:
                ...
                if rows_c1 :
                    ...
                    if SelOpt(yesNo).startOpt():   
                        super().prodHisDelete(in_code) # 2023-03-21 / 이력 테이블 데이터 추가
                        sql = f"DELETE FROM product WHERE pCode = '{in_code}'" # 테이블 레코드 삭제 SQL 
                        sqlAction(sql, True) # 2023-03-02 접속함수 실행      
                        # history 테이블 데이터 추가 / 이전 데이터 가져와서 활용
                        print('삭제 성공했습니다.')
                        Read('p').readAll() 
                        break
            ...
            while True:
                ...
                if rows_c1 : 
                    ...
                    if SelOpt(yesNo).startOpt(): 
                        ...
                        super().prodHisUpdate(in_code, iValue) # 2023-03-21 / 이력관리 테이블 추가 
