...
# 588라인 사용자 입력 함수
# 127라인 입력값 수정
# 208, 262라인 테이블 중복값 처리 --> 일단 기존 방식으로 처리해서 등록후 필터 점검하기 
# 요구사항5 ,6 의 map zip함수 처리 
# zip함수는 두개의 연속형 변수를 묶는 것
# map함수는 요소마다 함수 적용
...
class SelOpt() :
    ...
    def getOpt(self) : # 작업(옵션)번호 획득 메서드.
        if findall('[ㅏ-ㅣ]', self.option): # 한글을 입력했을 경우
            ### <요구사항-6> 반영 ###
            reop_dict = dict(map(reversed, self.op_dict.items())) # dict의 key, value를 변경하는 코드                                                
    ...
    def startOpt(self) : # 입력값 반환 메서드.
        if self.select in self.eng_lst : return self.select
        else : return 0 
...
class FilePreProcessing(FileInput):
    ...
    def readFile(self):
        try:
            ...
            print(e)
            print('경로 : ', f"{file_path}/{file_name}", '해당 경로에 입력한 파일이 없습니다. 다시 확인해주세요.')
    ...
    def saleReadFile(self):
        try:
            ...
            print(e,'\n경로 : ', path, '해당 경로에 입력한 파일이 없습니다. 다시 확인해주세요.')
...
# 윤년, 월, 일 확인 
    ...
    def tableChk(self, contents):
        try:
            ...
            data = tuple(map(str, [i for content in contents for i in content])) # 매개변수 전처리
    ...
    def fileInsert(self):
        try:
            ...
            if content_duplicate_chk == 1:
                ...
                # 0 --> 제품코드 / 1 --> 날짜 / 2--> 수량 / 필요한거 unitPrice
            else:
...
class InputFilter() : 
    ...
    # 2023-02-13 / 2023-03-02 매서드로 위치변경 
    ...
    def setDiscRate(self, discrate) :
        ...
        # 1) 공백을 입력한 경우 / # 2023-03-01 할인율 필터 추가
        if discrate == '' or len(discrate) < 0 :
...
class FindSql(UserInput):
    table = ''
    table_name = ''
    def __init__(self,table, find_sel): # 매개변수 (테이블, 찾을 SQL번호) / ### <요구사항-3> 반영 ### 
        super().__init__()
        self.table = table
        ...
        try :
            while True:
                ...
                # 2) 판매 테이블
                    ...
                    SaleFunc().saleReadAll()  # SQL관리 클래스 호출 및 sales테이블의 전체 데이터 출력  
    ...
    def prodCreate(self):
        try :
            while True:
                ...
                # 코드 입력값이 공백이 아닌 경우   
                if in_code != '' :
                    rows = FindSql('p',3).sqlFind(in_code) # 조건에 맞는 SQL실행 
    ...
    def prodDelete(self) : 
        try :
            while True:
                ...
                rows_c1 = FindSql('p',3).sqlFind(in_code) # 조건에 맞는 SQL실행
    ...
    def prodUpdate(self) : 
        try :
            while True:
                ...
                print("<<<제품 수정입니다>>>\n수정할 데이터의 ",end='') 
                in_code = super().codeInput() 
                rows_c1 = FindSql('p',3).sqlFind(in_code)
                if rows_c1 : 
                    print("<<<제품코드 조회결과입니다>>>")
...
# (1) 판매기록 등록 매서드      
    def saleCreate(self) :
        try :
            while True:
                ...
                print("<<<판매기록 등록입니다>>>\n등록할 데이터의 ",end='')
                in_code = super().codeInput() 
                rows_c1 = FindSql('s',3).sqlFind(in_code)
                # 코드 입력값이 공백이 아닌 경우 
                if in_code != '' :         
                    ...
                    # 2) product 테이블에 해당 제품코드의 상품이 존재하는 경우
                        ...
                        rows_c3 = FindSql('s',1).sqlFind(in_code)
                        unitPrice = rows_c3[0][0] ; qty = super().qtyInput() # 제품 가격 출력 / 2023-02-08
                        total_price = get_Price(qty, unitPrice) # 2023-03-02 가격계산함수 사용
                        sql = f"INSERT INTO sales(sCode, Qty, Amt) VALUES('{in_code}','{qty}', '{total_price}')" 
                        sqlAction(sql, True) # 2023-03-02 접속함수 실행
    ...
    def saleDelete(self) : 
        try :
            while True:
                ...
                print('삭제할 데이터의 ',end='') 
                in_code = super().codeInput() 
                rows = FindSql('s', 3).sqlFind(in_code)
                # 1) 제품코드가 존재하는 경우
                if rows : 
                    ...
                    print('삭제할 데이터의 ',end='')
                    in_no = super().noInput()
                    rows1 = FindSql('s',2).sqlFind(in_no)
                    # 1-1) 제품코드가 존재하면서 No가 존재하는 경우
                    if rows1:
                        # 판매 삭제 확인문
                        while True:
                            yesNo = input('삭제하시겠습니까>(y/n) : ')
                            if (SelOpt(yesNo).startOpt() == "Y") and rows1: 
                                ...
                                break
                            # 판매 확인문 취소 입력
                            elif (SelOpt(yesNo).startOpt() == "Y") and not rows1:
    ...
    def saleReadAll(self) :
        print("<<<판매 목록 조회입니다>>>")
        rows_c1 = FindSql('s', 0).sqlFind()
        ShowTables().showSale(rows_c1)
    ...
    def saleUpdate(self) : 
        try :
            while True:
                ...
                if rows : 
                    print("<<<입력한 제풐코드의 판매기록 목록 조회입니다>>>")
                    ShowTables().showSale(rows)
                    print('수정할 데이터의 ',end='')
                    in_no = super().noInput()
                    rows1 = FindSql('s',2).sqlFind(in_no) # 수정할 데이터의 No선택
                    # 판매 수정 확인문
                    yesNo = input('수정하시겠습니까>(y/n) : ')
                    if (SelOpt(yesNo).startOpt() == "Y") and rows1: 
                        print("<<<수정할 데이터를 입력하세요.>>>")
                        in_qty = super().qtyInput()                   
                        rows = FindSql('s', 1).sqlFind(rows1[0][0]) # 제품코드를 이용하여 제품의 가격과 할인율을 가져오는 SQL / 2023-02-09
                        # 구매 수량에 따른 총 가격 계산 
                        total_price = get_Price(in_qty, rows[0][0])
...
# <변경사항> 2023-02-28
# FindSql 클래스에서 분리
def sqlAction(sql, com=False) : # 매개변수 : sql문, Commit 결정
    try :
        conn = pymysql.connect(**config)    
        cursor = conn.cursor()
        cursor.execute(sql)
...
def get_Price(in_Code, qty):
    while True:
        sql = f"select unitPrice from product where pCode = '{in_Code}'" # product 테이블에서 row_data에 맞는 제품가격을 불러온다.
        pamt_tuples = sqlAction(sql)
        in_amt = ''.join(map(str,[pamt for pamt_tuple in pamt_tuples for pamt in pamt_tuple]))
        print(in_amt)
        # yesNo = input('판매가격에 할인율을 적용하시겠습니까? (y/n) : ')
        # if SelOpt(yesNo).startOpt() == "Y": # <1> 할인율이 적용된 가격 계산 
        #     dis_price = round((unitPrice * (100 - round(disrate)))/100)
        #     total_price = dis_price * int(qty) 
        #     break
        # elif SelOpt(yesNo).startOpt() == "N": # <2> 할인율이 적용되지 않은 가격 계산 <요구사항>                    
        #     total_price = unitPrice * int(qty) # 할인율이 적용되지 않은 총 가격 
        #     break
        # else:
        #     print('잘못 입력했습니다. 다시 입력해주세요.')
        #     continue
        ...
        # 판매 관리 서비스
            ...
            while True:
                ...
                if sel == 1 :
                    ...
                    FindSql('s','코드').sqlReadOne()
                    os.system("pause")
                elif sel == 4 :
                    FindSql('s','제품명').sqlReadOne()
                    os.system("pause")
                elif sel == 5 :
                    c0.saleUpdate()
                    os.system("pause")
