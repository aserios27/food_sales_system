...
from Filter2 import UserInput, FindSql, SelOpt, ShowTables, TableReset, Read, sqlAction
            ...
            while True:
                ...
                if rows_c1 :
                    ...
                    if SelOpt(yesNo).startOpt():
                        sql = f"DELETE FROM product2 WHERE pCode = '{in_code}'" # 테이블 레코드 삭제 SQL 
                        sqlAction(sql, True) # 2023-03-02 접속함수 실행      
                        print('삭제 성공했습니다.')
                        Read('p').readAll() 
                        break
            ...
            while True:
                ...
                if rows_c1 :
                    ...
                    if SelOpt(yesNo).startOpt(): 
                        ...
                        sql = f"UPDATE product2 SET pName = '{iValue[0]}', UnitPrice = '{iValue[1]}', discountRate ='{iValue[2]}' WHERE pCode = '{in_code}'"
                        sqlAction(sql, True) # 2023-03-02 접속함수 실행      
                        print("수정을 완료했습니다\n수정 결과입니다")
                        Read('p').readAll() 
                        break
