...
class FilePreProcessing(FileInput):
    ...
    def readFile(self):
        try:
            print('등록 가능 데이터 예시'); print('-'*40); print('a001,20210701,5'); print('-'*40)
...
class FileInsert(FileTextFilter, FilePreProcessing):    
    def __init__(self):
        super().__init__()
        self.fileInsert() # fileInsert 매서드 자동 실행
...
class GraphsPreProcessing:
    ...
    def salDataCreate(self): 
        ...
        print('저장할',end=''); path = c1.pathInput()
        print('저장할',end=''); file_name = c1.nameInput()
    ...
    def salPreProcessing(self):
        try:
            ...
            cursor = conn.cursor() 
            c1 = FileInput()
            print('사용할',end=''); path = c1.pathInput()
            print('사용할',end=''); file_name = c1.nameInput()
        ...
        try :
            while True:
                ...
                if self.read_sel == '코드' :
                    ...
                    print("조회할 ",end='')
                    in_code = UserInput().codeInput()
                # 2) 제품명 해당하는 데이터를 가져올 경우
                elif self.read_sel == '제품명' : 
                    print("<<<제품 개별 조회({})입니다>>>".format(self.read_sel))
                    print("조회할 ",end='')
                    in_code = UserInput().nameInput()
