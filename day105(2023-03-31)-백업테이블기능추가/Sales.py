...
# (1) 판매기록 등록 매서드      
    ...
    def abolitionSaleRead(self) :
        print(f"<<<판매제품 조회(삭제내역) 입니다>>>")
        sql = "SELECT DISTINCT s.seqNo, s.sCode, p.pName, date_format(s.sDate,'%Y%m%d') as date, s.qty, s.amt FROM sales_backup AS s LEFT OUTER JOIN product_history AS p ON s.sCode = p.Code WHERE s.Event = '단종';"
        rows = sqlAction(sql)
        ShowTables().showSale(rows)
...
# 기존 세무조사 매서드와 주주총회 매서드 분리 후 독립된 클래스화 
    ...
    def commonTask(self):
        try :
            ...
            sql = f"SELECT unitPrice FROM product_history WHERE Code = '{in_code}'"
            rows = sqlAction(sql)
...
def totalRead():
    try :
        print(f"=== 회계팀용 판매 목록 조회 ===")
        sql = "SELECT DISTINCT s.seqNo, s.sCode, p.pName, date_format(s.sDate,'%Y.%m.%d') as date, s.qty, s.amt FROM all_sales AS s LEFT OUTER JOIN product_history AS p ON s.sCode = p.Code ORDER BY seqNo"
        rows = sqlAction(sql)
        ShowTables().showSale(rows)
