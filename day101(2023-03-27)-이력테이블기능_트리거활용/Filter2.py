    ...
    def __init__(self,table, find_sel, find_in_data = ''): # 매개변수 (테이블, 찾을 SQL번호, 조건값) / ### <요구사항-3> 반영 ### 
        ...
        if self.table.lower() == 'p':
            ...
            if find_sel == 3 or find_sel == '코드': 
                self.find_sql = f"SELECT * FROM product2 WHERE pCode = '{find_in_data}'"
            # 2) 제품명를 이용하여 product테이블의 해당 데이터 호출
            elif find_sel == 4 or find_sel == '제품명': 
                self.find_sql = f"SELECT * FROM product2 WHERE pName like '%{find_in_data}%'" 
            # 3)product테이블의 전체 데이터 호출
            elif find_sel == 0:
                self.find_sql = "SELECT * FROM product2"
                
        # 판매 테이블 sql
        elif self.table.lower() == 's':
            # 1) 제품코드로 판매관리 필요 데이터 전체조회
            if find_sel == 3 or find_sel == '코드': 
                self.find_sql = f"SELECT s.seqNo, s.sCode, p.pName, date_format(s.sDate,'%Y.%m.%d') as date, s.qty, s.amt FROM sales2 AS s JOIN product2 AS p ON s.sCode = p.pCode WHERE p.pCode = '{find_in_data}'" 
            # 2) 제품명으로 판매관리 필요 데이터 전체조회
            elif find_sel == 4 or find_sel == '제품명': 
                self.find_sql = f"SELECT s.seqNo, s.sCode, p.pName, date_format(s.sDate,'%Y.%m.%d') as date, s.qty, s.amt FROM sales2 AS s JOIN product2 AS p ON s.sCode = p.pCode WHERE p.pName like '%{find_in_data}%'"
            # 3) 판매관리 필요 데이터 전체조회
            elif find_sel == 0:
                self.find_sql = "SELECT s.seqNo, s.sCode, p.pName, date_format(s.sDate,'%Y%m%d') as date, s.qty, s.amt FROM sales2 AS s LEFT OUTER JOIN product2 AS p ON s.sCode = p.pCode"
            # 4) product테이블에서 제품가격과 할인율 가져오기
            elif find_sel == 1:
                self.find_sql = f"SELECT unitPrice, discountRate FROM product2 WHERE pCode = '{find_in_data}'" 
            # 5) 인덱스로 제품코드 조회
            elif find_sel == 2:
                self.find_sql = f"SELECT sCode FROM sales2 WHERE seqNo = '{find_in_data}'"
        else:
    ...
    def readOne(self, read_sel) :
        ...
        try :
            while True:
                ...
                if len(rows) > 0 :
                    os.system('cls')
                    print(f"===트리거활용 테이블 조회2({self.read_sel})===")
                    if self.table == 'p':
                        ShowTables().showProd(rows)
                        break
                    elif self.table == 's':
                        ShowTables().showSale(rows) ; print('-'*90)
...
class TableCreate:
    ...
    def productTableCreate(self) :  
        try :
            conn = pymysql.connect(**config)
            cursor = conn.cursor()
            sql = """CREATE table if not exists product2 (
    ...
    def salesTableCreate(self) :  
        try :
            conn = pymysql.connect(**config)
            cursor = conn.cursor()
            sql = """CREATE table if not exists sales2 (
...
class TableReset:
    ...
    def prodReset(self):
        ...
        sql = "INSERT INTO product2(pCode, pName, unitPrice, discountRate) VALUES(%s, %s, %s, %s);"
    ...
    def saleReset(self):
        ...
        sql = "INSERT INTO sales2(sCode, sDate, Qty, Amt) VALUES(%s, %s, %s, %s);"
            ...
            while True:
                ...
                if SelOpt(yesNo).startOpt():  
                    try:
                        ...
                        # 테이블 전체 내용 삭제
                        sql = f"DELETE FROM {self.table_name}2"
                        sqlAction(sql, True)
                        # 대상이 sales 테이블일경우 seqNo가 1부터 시작되도록 초기화
                        if self.table == 'p' :
                            sql = f"DELETE FROM {self.table_name}_history2" # 2023-03-21 / 접속함수 실행
                            sqlAction(sql, True)
                            sql = f"ALTER TABLE {self.table_name}_history2 auto_increment = 1" # 2023-03-02 / seqNo 컬럼 초기화
                            sqlAction(sql, True) 
                        elif self.table == 's' :
                            sql = "ALTER TABLE sales2 auto_increment = 1"
                            sqlAction(sql, True) # 2023-03-02 / 접속함수 실행
                            
                        yesNo = input('제품테이블에 기존 데이터를 추가하시겠습니까?(y/n) : ')
                        if SelOpt(yesNo).startOpt() and self.table == 'p': self.prodReset()
