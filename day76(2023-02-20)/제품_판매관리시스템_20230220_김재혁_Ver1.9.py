    ...
    def sqlReadOne(self) :
        try :
            while True:
                ...
                # 1) 제품테이블
                if self.table == 'p' :   
                    c0 = ProdFunc()  # 제품 관리 클래스 호출
                    c0.prodReadAll() # 제품 테이블 전체 출력 / 2023-02-09
                    c1 = UserInput() # 사용자 입력 클래스
                ...
                # 2) 판매 테이블
                    ...
                    c0 = SalFunc()   # sales관리 클래스 호출
                    c0.salReadAll()  # sales테이블의 전체 데이터 출력  
                    c1 = UserInput() # 사용자 입력 클래스 호출
            ...
            while True:
                ...
                if in_code != '' :
                    ...
                    #2) 같은 제품코드가 존재하지 않는 경우
                        ...
                        iValue = c2.userInput() # 사용자 입력값 호출 
...
class TableReset:
    ...
    def tableReset(self):
        ...
        if self.table == 'p':
            c1 = ProdFunc()
            find_rows = c1.prodReadAll()
        elif self.table == 's':
            c1 = SalFunc().salReadAll() 
        ...
        try :
            while True:
                ...
                if in_code != '' :
                    ...
                    if len(rows_c1) == 0:
                        iValue = c2.userInput() # 사용자 입력 값 호출
                        sql = f"INSERT INTO product(pCode, pName, UnitPrice, discountRate) VALUES('{in_code}','{iValue[0]}', '{iValue[1]}', '{iValue[2]}')" 
                        cursor.execute(sql)
                        conn.commit()
                        print("제품등록을 성공했습니다.")
                    ...
                    #2) product 테이블에 해당 제품코드의 상품이 존재하는 경우
                        ...
                        in_qty = c2.qtyInput() # 사용자 입력 함수 호출
                        # <1>할인율이 적용된 총 가격
                        # total_price = dis_price * int(iValue[0])
                        
                        # <2>할인율이 적용되지 않은 총 가격
                        total_price = price * int(in_qty)
...
#--------------------------
# 여기서 부터 알파벳 순서가 안맞는 이유
# 클래스를 알파벳 순으로 정의 후 상속을 하려고 하니
# 클래스가 정의되어 있지않다는 오류가 발생화여 
# 부득이하게 정렬 순서를 어김
#--------------------------  
# 파일 경로 입력 분리 / 2023-02-21
class FileFilter():
    def __init__(self):
...
class FilePreProcessing(FileInput):
    ...
    def readFile(self):
        try:
            c0 = FileInput()
            file_path = c0.PathInput()
            file_name = c0.NameInput()
            ...
            print(e)
            print('경로 : ', path)
            print('해당 경로에 입력한 파일이 없습니다. 다시 확인해주세요.')
        finally :
            t1.close()
...
# 윤년, 월, 일 확인 
    ...
    def yearChk(self, st) :
        ...
        if flag2 == True and year_st == '평년':
            if ls_month[int(month)-1] < int(day):
                flag4 = False
            else:
                flag4 = True
        if flag2 == True and year_st == '윤년':
            if ls_month[month-1] < day:
                flag4 = False
            else:
                flag4 = True
...
class FileInsert():    
    ...
    def fileInsert(self):
        try:
            ...
            cursor = conn.cursor()
            print(f'\n<<<제품 테이블에 외부 파일 등록입니다>>>')
            c0 = FilePreProcessing()
            c1 = FileTextFilter()
            content = c0.readFile()
            print(content)
            
            # 1) 년월일 필터
            content_year_chk = c1.yearFilter(content)

            # 2) 제품코드 필터
            content_code_chk = c1.codeFilter(content_year_chk)

            # 3) 테이블 중복 확인 필터
            st = c1.tableChk(content_code_chk)
...
def salAllCreate(sel): 
    try:
        conn = pymysql.connect(**config)
        cursor = conn.cursor()    
        sql = "SELECT seqNo, sCode, date_format(sDate,'%Y%m%d') as date, Qty, Amt FROM sales"
        cursor.execute(sql)
        rows = cursor.fetchall()
        ls_t1 = []
        for row in rows:
        ...
        df.to_csv(f'{path}/sales_all.txt', sep = ',', header = False, index=False)
        t1 = open(f'{path}/sales_all.txt', mode = 'r', encoding='utf8')
        test_t1 = t1.read()
        temp1_t1 = findall('[^,\n]+',test_t1)
        ls_t1 = []
        for i in range(len(temp1_t1)):
            if i % 5 == 0:
                ls_t1.append(((temp1_t1[i+1],)+(temp1_t1[i+2],)+(temp1_t1[i+3],)+(temp1_t1[i+4],)))

        # 년월일 오류 체크
        test1_ls_t1 = [] # 오류 없는 데이터
        chk_ls1 = []      # 오류 데이터 인덱스
        for i in range(len(ls_t1)):
            test1_ls_t1.append(yearChk(ls_t1[i][1]))
            if test1_ls_t1[i] == '':
                chk_ls1.append(i)
                ls_t1.remove(ls_t1[i])

        # 제품코드 확인
        chk_ls2 = [] # 오류 없는 데이터
        # print(test1_ls_t1) 
        for i in range(len(test1_ls_t1)):
            sql = f"SELECT unitPrice FROM product WHERE pCode = '{ls_t1[i][0]}'"
            cursor.execute(sql)
            rows = cursor.fetchall()
            # 제품코드가 존재한다면
            if rows:
