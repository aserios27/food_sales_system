...
class TableCreate:
    def __init__(self, table):
        self.table = table
        if self.table == 'p': self.prodCreate()
    ...
    def salesTableCreate(self) :  
        try :
            ...
            sql = """CREATE table if not exists sales(
                ...
                FOREIGN KEY (sCode) REFERENCES product(pCode)
                ON DELETE CASCADE)
                """
    ...
    def salebackupCreate(self) :  
        try :
            conn = pymysql.connect(**config)
            cursor = conn.cursor()
            sql = """CREATE TABLE if not exists sales_backup (
                seqNo int NOT NULL,
                sCode varchar(4) NOT NULL,
...
class TableReset:
    ...
    def prodReset(self):
        ...
        sql = "INSERT INTO product_history(Code, Event, pName, unitPrice, discountRate) VALUES (%s, 'Insert', %s, %s, %s);"
        sqlAction(sql, True, False, prod_insert)
