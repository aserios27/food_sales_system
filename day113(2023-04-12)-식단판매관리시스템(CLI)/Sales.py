...
from Filter import UserInput, FindSql, SelOpt, ShowTables, TableReset, Read, sqlAction, fmt
...
class SaleFunc(UserInput, HisFunc) :
    def __init__(self):
        super().__init__()
# (1) 판매기록 등록 매서드      
    def saleCreate(self) :
        try :
            while True:
                ...
                if in_code != '' :         
                    ...
                    # 2) product 테이블에 해당 식단코드의 상품이 존재하는 경우
                    else :
                        qty = super().qtyInput() # 식단 가격 출력 / 2023-02-08 / 
                        sql = f"INSERT INTO menu_sales(sCode, Qty, Amt) VALUES('{in_code}','{qty}', '{getPrice(in_code, qty)}')" 
                        sqlAction(sql, True) # 2023-03-02 접속함수 실행
    ...
    def saleUpdate(self) : 
        try :
            while True:
                ...
                print('<<<판매기록을 수정합니다>>>\n수정할 데이터의 ',end='')
                in_code = super().codeInput() # 식단코드 입력 매서드 호출
                rows = FindSql('s', 3, in_code).sqlFind()

                # SELECT 쿼리 실행결과가 있는 경우               
                if rows :
                    os.system('cls') 
                    print("<<<입력한 식단코드의 판매기록 목록 조회입니다>>>")
                    ShowTables().showSale(rows)
                    print('수정할 데이터의 ',end='')
                    in_no = super().noInput()
                    ...
                    if SelOpt(yesNo).startOpt() and len(rows1) != 0: 
                        ...
                        sql = f"UPDATE menu_sales SET Qty = '{in_qty}', Amt = '{getPrice(in_code ,in_qty)}' WHERE seqNo = '{in_no}'"
                        sqlAction(sql, True) # 2023-03-02 /접속함수 실행
                        os.system('cls')       
                        print("<<<수정을 완료했습니다>>>\n<<<수정 결과입니다>>>")
                        Read('s').readAll()
    ...
    #--------------------------              
    def abolitionSaleRead(self) :
        print(f"<<<판매식단 조회(삭제내역) 입니다>>>")
        sql = "SELECT DISTINCT s.seqNo, s.sCode, p.pName, date_format(s.sDate,'%Y%m%d') as date, s.qty, s.amt FROM menu_sales_backup AS s LEFT OUTER JOIN menu_planner_history AS p ON s.sCode = p.pCode WHERE s.Event = '단종';"
        rows = sqlAction(sql)
        ShowTables().showSale(rows)
...
# 기존 세무조사 매서드와 주주총회 매서드 분리 후 독립된 클래스화 
    ...
    def commonTask(self):
        try :
            ...
            sql = f"SELECT unitPrice FROM menu_planner_history WHERE pCode = '{in_code}'"
            rows = sqlAction(sql)
            if self.task == '재무담당자': self.sel_unitPrice = int(min([i for row in rows for i in row]))
            elif self.task == '인사담당자': self.sel_unitPrice = int(max([i for row in rows for i in row]))
            rows = FindSql('s',3, in_code).sqlFind()
            os.system('cls')
            print(f"=== 판매조회2(코드별-{self.task}용)===")
            print(f"{fmt('No',6,'l')}{fmt('식단코드',10,'l')}{fmt('식단명',20,'l')}{fmt('판매등록일자',22,'l')}{fmt('판매수량',16,'l')}{fmt('총 판매가격',5,'l')}") ; print('-'*90)
            sum_amt = 0
            for row in rows :
...
def totalRead():
    try :
        print(f"=== 재무담당자용 판매 목록 조회 ===")
        sql = "SELECT DISTINCT s.seqNo, s.sCode, p.pName, date_format(s.sDate,'%Y.%m.%d') as date, s.qty, s.amt FROM menu_all_sales AS s LEFT OUTER JOIN menu_planner_history AS p ON s.sCode = p.pCode ORDER BY seqNo"
        rows = sqlAction(sql)
        ShowTables().showSale(rows)
...
def getPrice(in_Code, qty):
    sql = f"SELECT unitPrice, discountRate FROM menu_planner WHERE pCode = '{in_Code}'" # product 테이블에서 row_data에 맞는 식단가격을 불러온다.
    pamt_tuples = sqlAction(sql)
    in_unitPrice = list(map(int,[pamt for pamt_tuple in pamt_tuples for pamt in pamt_tuple]))[0] 
    in_disrate = list(map(int,[pamt for pamt_tuple in pamt_tuples for pamt in pamt_tuple]))[1]
    ...
    while True:
        ...
        if sel == '1' :
            ...
            while True:
                ...
                print("판매조회(인사담당자)      : 2  ")
                print("영양사   판매목록   조회  : 3  ")
                print("재무담당자 판매목록 조회  : 4  ")
                print("판매식단 조회(삭제내역)   : 5  ")
                print("뒤로               가기   : 0  ")  
                sel = input("작업을 선택하세요 : "  )
                os.system('cls')
                if sel == '1':
